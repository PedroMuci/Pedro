...
@section('title', 'Nova Competição')

@section('content')
<div class="max-w-2xl mx-auto bg-[var(--card-bg)] p-6 rounded shadow-md border border-[var(--border-color)]">
  <h2 class="text-2xl font-semibold mb-6 text-center">Criar Nova Competição</h2>

  <form method="POST" action="{{ route('competicoes.store') }}" class="space-y-4">
    @csrf
    ...
    <div>
      ...
      <input type="url" name="icone" id="icone" value="{{ old('icone') }}"
             class="input w-full p-2 border rounded bg-transparent text-[var(--text-color)] border-[var(--border-color)]" required>
    </div>
    ...
    <div>
      ...
      <select name="tipo" id="tipo"
        ...
        <option value="liga" @selected(old('tipo') === 'liga')>Liga</option>
        <option value="copa" @selected(old('tipo') === 'copa')>Copa</option>
      </select>
    </div>

    <div>
      <label for="data_criacao" class="block mb-1 font-semibold">Data de Criação</label>
      <input type="date" name="data_criacao" id="data_criacao" value="{{ old('data_criacao') }}"
             class="input w-full p-2 border rounded bg-transparent text-[var(--text-color)] border-[var(--border-color)]" required>
    </div>

    <div>
      <label for="edicao" class="block mb-1 font-semibold">Número da Edição</label>
      <input type="number" name="edicao" id="edicao" value="{{ old('edicao') }}"
             class="input w-full p-2 border rounded bg-transparent text-[var(--text-color)] border-[var(--border-color)]" required>
    </div>
